...
export class NodeDataCtrl extends PanelCtrl {
  ...
  loadCluster() {
    ...
    if (!cluster) {
      ...
      const heiliDS  = _.find(this.templateVariables, {'name': 'datasource'}).current.value;

      this.loadDatasource(cluster_id).then(() => {
        return this.nodeStatsDatasource.getNodeStats(cluster_id, heiliDS);
      }).then(nodeStats => {
        if (node_name === 'All') {
  ...
  loadDatasource(id) {
    return this.backendSrv.get('api/datasources')
      .then(result => {
        return _.filter(result, {"type": "heili-kubernetes-datasource", "name": id})[0];
      })
      .then(ds => {
        if (!ds) {
